...
const Toast = ({ message, type, onClose }) => {
  ...
  return (
    <div style={{
        ...
        zIndex: 9999, animation: "slideIn 0.3s ease-out", maxWidth: "400px",
      }}>
  ...
  return (
    <div style={{
      ...
      <div style={{
          ...
          display: "flex", flexDirection: "column", boxShadow: COLORS.shadowHuge
      }}>
        <div style={{ padding: "24px", borderBottom: `1px solid ${COLORS.borderColor}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
                <h3 style={{ margin: 0, fontSize: "18px", color: COLORS.textPrimary }}>Responses</h3>
        ...
        <div style={{ padding: "24px", overflowY: "auto" }}>
            {assignment.responses && assignment.responses.length > 0 ? (
                assignment.responses.map((resp, idx) => (
                    <div key={idx} style={{ marginBottom: "24px", backgroundColor: "#f9fafb", padding: "16px", borderRadius: "12px" }}>
                        <div style={{ fontSize: "14px", fontWeight: "600", color: COLORS.textPrimary, marginBottom: "8px" }}>
                            Q{idx+1}: {resp.question_text}
                        </div>
                        <div style={{ fontSize: "14px", color: COLORS.textSecondary, whiteSpace: "pre-wrap" }}>
                            {resp.answer_text}
                        </div>
                    </div>
                ))
            ) : (
                <div style={{ textAlign: "center", color: COLORS.textMuted, padding: "20px" }}>No responses found.</div>
            )}
        </div>
        
        <div style={{ padding: "20px", borderTop: `1px solid ${COLORS.borderColor}`, textAlign: "right" }}>
...
const SurveyHistoryCard = ({ survey, onViewDetails }) => {
  ...
  const userCountLabel = survey.recipient_count ? `${survey.recipient_count} Recipients` : "All Users";
  ...
  return (
    <div style={styles.historyCard}>
      <div style={styles.historyHeader}>
        ...
        <h3 style={{ margin: "8px 0", fontSize: "18px", fontWeight: "700", color: COLORS.textPrimary }}>{survey.title}</h3>
        <div style={{ display: 'flex', gap: '16px', fontSize: '13px', color: COLORS.textSecondary }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><Calendar size={14} /> {dateStr}</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><Users size={14} /> {userCountLabel}</div>
        </div>
      </div>
...
const SurveyDetailsView = ({ surveyId, onBack, API_BASE, authHeader }) => {
    ...
    if(loading) return <div style={{textAlign:"center", padding:"40px"}}><Loader2 className="animate-spin" /> Loading...</div>;
    ...
    return (
        <div>
            ...
            <button onClick={onBack} style={{...styles.btnSecondary, marginBottom: "20px", border: "none", paddingLeft: 0}}>
                <ArrowLeft size={18} /> Back to History
            </button>

            <div style={{marginBottom: "30px"}}>
                <h2 style={{fontSize: "28px", fontWeight: "800", color: COLORS.textPrimary, marginBottom: "8px"}}>{details.title}</h2>
                <div style={{display: "flex", alignItems: 'center', gap: "12px", fontSize: "14px", color: COLORS.textSecondary}}>
                    {/* ✅ Badges with spacing */}
                    ...
                    <span>•</span>
                    <span>Date: <strong>{new Date(details.created_at).toLocaleDateString()}</strong></span>
                </div>
            </div>

            <div style={{...styles.card, overflowX: "auto"}}>
                <h3 style={{fontSize: "18px", fontWeight: "700", marginBottom: "16px"}}>Recipients Status</h3>
                    ...
                    <tbody>
                        {details.assignments.map((assign, index) => (
                            <tr key={assign.id}>
                                ...
                                {!isAnonymous && (
                                    <td style={styles.td}>{assign.user_email}</td>
                                )}
                                
                                <td style={styles.td}>
                                    <span style={styles.statusBadge(assign.status)}>{assign.status}</span>
                                </td>
                                <td style={styles.td}>
                                    {assign.status === 'completed' ? (
                                        <button 
                                            onClick={() => setSelectedAssignment({
                                                ...
                                                user_name: isAnonymous ? `Employee #${index + 1}` : assign.user_name
                                            })}
                                            style={{
                                        ...
                                        <span style={{color: COLORS.textMuted, fontSize: "12px", fontStyle: "italic"}}>No answers yet</span>
                                    )}
                                </td>
                            </tr>
                        ))}
                        {details.assignments.length === 0 && (
                            <tr><td colSpan={isAnonymous ? "3" : "4"} style={{...styles.td, textAlign: "center"}}>No recipients found.</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
...
const CreateSurveyForm = () => {
  ...
  useEffect(() => {
    async function fetchAudienceData() {
      ...
      } catch (e) { console.error("Failed to load audience data", e); } 
  ...
  // Handlers
  const handleAddQuestion = () => setQuestions([...questions, { id: Date.now(), text: "" }]);
  const handleQuestionChange = (id, text) => setQuestions(questions.map((q) => (q.id === id ? { ...q, text } : q)));
  ...
  const handleSubmit = async () => {
    ...
    if (validQuestions.length === 0) {
        return setToast({ message: "Please add or import at least one question.", type: "error" });
    }

    setIsSubmitting(true);
    const payload = {
      title, method: option, response_type: responseType,
    ...
    try {
      ...
      if (response.ok) {
        ...
        setTitle(""); setQuestions([{ id: Date.now(), text: "" }]);
      } else {
      ...
      <div className="survey-builder-main-wrapper" style={styles.mainWrapperCard}>
        ...
        {view !== 'details' && (
            ...
            <div>
                ...
                {view === 'create' ? "Design custom assessments & notify employees." : "Track status and view employee responses."}
                </p>
            </div>
            <button style={styles.btnSecondary} onClick={() => setView(view === 'create' ? 'history' : 'create')}>
                {view === 'create' ? <><List size={18} /> View History</> : <><Plus size={18} /> Create New</>}
            </button>
            </div>
        )}
        ...
        {view === 'history' && (
            <div>
                {loadingHistory ? (
                    <div style={{textAlign: "center", padding: "40px"}}><Loader2 className="animate-spin" /> Loading...</div>
                    ...
                    <div className="history-grid" style={styles.historyGrid}>
                        {historyList.map(survey => (
                            <SurveyHistoryCard key={survey.id} survey={survey} onViewDetails={handleViewDetails} />
                        ))}
                    </div>
                )}
            </div>
        ...
        <div className="survey-builder-content-wrapper" style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
          ...
          <div className="survey-builder-card" style={styles.card}>
            <label className="survey-builder-label" style={styles.label}><Edit3 size={18} color={COLORS.primary} /> Assessment Title</label>
            <input style={styles.input} type="text" placeholder="e.g. Q1 Employee Wellness Check" value={title} onChange={(e) => setTitle(e.target.value)} />
          </div>

          {/* Methodology */}
          <div className="survey-builder-choice-wrapper" style={{ display: "flex", gap: "20px" }}>
            <div className="survey-builder-choice-card" style={styles.choiceCard(option === "upload")} onClick={() => setOption("upload")}>
              <Upload size={28} color={option === "upload" ? COLORS.primary : COLORS.textMuted} />
              <div style={{ textAlign: "center" }}>
                <div style={{fontWeight: "700", color: option === "upload" ? COLORS.primaryDark : COLORS.textPrimary}}>Upload File</div>
                <div style={{fontSize: "12px", color: COLORS.textSecondary}}>Import PDF or Excel</div>
              </div>
            </div>
            <div className="survey-builder-choice-card" style={styles.choiceCard(option === "manual")} onClick={() => setOption("manual")}>
              <Edit3 size={28} color={option === "manual" ? COLORS.primary : COLORS.textMuted} />
              <div style={{ textAlign: "center" }}>
                <div style={{fontWeight: "700", color: option === "manual" ? COLORS.primaryDark : COLORS.textPrimary}}>Build Manually</div>
                <div style={{fontSize: "12px", color: COLORS.textSecondary}}>Write custom questions</div>
              </div>
            </div>
          </div>
          ...
          <div className="survey-builder-card" style={styles.card}>
            {option === "upload" ? (
              <div>
                <label className="survey-builder-label" style={styles.label}><FileText size={18} color={COLORS.primary} /> Document Upload</label>
                <div style={{padding: "40px", border: `2px dashed ${COLORS.borderColor}`, borderRadius: "12px", textAlign: "center"}}>
                  <UploadSurveyFile onQuestionsImported={(importedQuestions) => {
                      setQuestions(importedQuestions); // ✅ Populate questions state from file
                      setToast({ message: `Imported ${importedQuestions.length} questions!`, type: 'success' });
                  }} />
                </div>
                
                {/* ✅ Optional: Preview Imported Questions */}
                {questions.length > 0 && questions[0].text !== "" && (
                    <div style={{marginTop: "20px", textAlign: "left", maxHeight: "200px", overflowY: "auto", background: "#f8fafc", padding: "10px", borderRadius: "8px"}}>
                        <div style={{fontSize: "12px", fontWeight: "700", color: COLORS.textSecondary, marginBottom: "8px"}}>Preview Imported Questions:</div>
                        {questions.map((q, i) => (
                            <div key={i} style={{fontSize: "13px", padding: "4px 0", borderBottom: "1px solid #e2e8f0"}}>
                                {i + 1}. {q.text}
                            </div>
                        ))}
                    </div>
              ...
              <div>
                <div style={{display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px"}}>
                  <label className="survey-builder-label" style={{ ...styles.label, marginBottom: 0 }}><Edit3 size={18} color={COLORS.primary} /> Question List</label>
                  <button className="survey-builder-btn-secondary" style={styles.btnSecondary} onClick={handleAddQuestion}><Plus size={16} /> Add Question</button>
                </div>
                {questions.map((q, i) => (
                  <div key={q.id} style={styles.questionRow}>
                    <div style={{fontWeight: "700", color: COLORS.textMuted, width: "30px"}}>{i + 1}.</div>
                    <input style={{...styles.input, border: "none", backgroundColor: "transparent", fontSize: "15px", fontWeight: "500"}} type="text" placeholder="Enter your question here..." value={q.text} onChange={(e) => handleQuestionChange(q.id, e.target.value)} />
                    {questions.length > 1 && <button onClick={() => handleRemoveQuestion(q.id)} style={{background: "none", border: "none", cursor: "pointer", color: COLORS.red, padding: "8px"}}><Trash2 size={18} /></button>}
                  </div>
                ))}
              </div>
            )}
          ...
          {/* Settings */}
          <div className="survey-builder-settings-grid" style={{display: "grid", gridTemplateColumns: "1fr 1fr", gap: "24px"}}>
            <div className="survey-builder-card" style={styles.card}>
              <label className="survey-builder-label" style={styles.label}><Layout size={18} color={COLORS.primary} /> Response Setup</label>
              <ResponseOptions value={responseType} onChange={setResponseType} />
            </div>
            <div className="survey-builder-card" style={styles.card}>
              <label className="survey-builder-label" style={styles.label}><Target size={18} color={COLORS.primary} /> Recipients</label>
              <AudienceSelector value={audience} onChange={setAudience} departments={departments} employees={employees} loading={loadingAudience} />
            </div>
          </div>

          {/* Schedule */}
          <div className="survey-builder-card survey-builder-schedule-section" style={{...styles.card, display: "flex", alignItems: "center", justifyContent: "space-between", backgroundColor: "#fcfcfd"}}>
            <div style={{display: "flex", alignItems: "center", gap: "16px"}}>
              <div style={{padding: "10px", backgroundColor: COLORS.primaryLight, borderRadius: "12px"}}><Calendar size={20} color={COLORS.primary} /></div>
              <div><div style={{fontWeight: "700", fontSize: "15px"}}>Delivery Schedule</div><div style={{fontSize: "13px", color: COLORS.textSecondary}}>Set a time or send immediately.</div></div>
            </div>
            <div className="survey-builder-schedule-container" style={{ width: "65%" }}>
              <div style={{padding: "16px 18px", borderRadius: "14px", border: `1px solid ${COLORS.borderColor}`, backgroundColor: "#fff"}}>
          ...
          {/* Submit */}
          <button className="survey-builder-btn-primary" style={{...styles.btnPrimary, opacity: isSubmitting ? 0.7 : 1, cursor: isSubmitting ? "not-allowed" : "pointer"}} onClick={handleSubmit} disabled={isSubmitting}>
            {isSubmitting ? <><Loader2 className="animate-spin" size={20} /> Sending...</> : <><CheckCircle2 size={20} /> Launch Custom Survey</>}
          </button>
        </div>
        )}
      </div>
